...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($_FILES['room_image']['name'])) {
        $errors['room_image'] = 'Room image is required';
    }
    ...
    if (empty($errors)) {
        ...
        $sql_room_add = "INSERT INTO `rooms` (`room_name`, `room_house_id`, `room_type`, `room_furnishing`, `room_price`, `room_bathroom`, `room_deposit`, `room_monthly_rental`, `room_description`, `room_image`) VALUES ('$room_name', '$room_house_id', '$room_type', '$room_furnishing', '$room_price', '$room_bathroom', '$room_deposit', '$room_monthly_rental', '$room_description', '$room_image_new_name')";

        $result_room_add = mysqli_query($conn, $sql_room_add);

        if ($result_room_add) {
            move_uploaded_file($room_image_tmp, $room_image_destination);
            set_flash_mesage('Room added successfully.', 'success');
            redirect('room.php');
        } else {
            set_flash_mesage('Room added failed.', 'danger');
            redirect('room.php');
        }
    }
}
